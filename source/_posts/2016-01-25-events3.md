...
;(function($){
    /*
     _zid            ：标识元素和回调函数的id，每标识一个+1
     slice           ：Array.prototype.slice原型方法缓存
     handlers        ：保存着所有的handler，结构如下，handlers对象的值为对应元素的_zid，值为对象数组
     {
     1: [ // 每个DOM元素的handler值对应该DOM元素的_zid
     {
     del: function() {}, // 实现事件代理的函数
     e: "click", // 事件名称
     fn: function() {}, // 用户传入的回调函数
     i: 0, // 该对象在数组里的下标
     ns: "", // 事件的命名空间，只有使用$.fn.triggerHandler时可用，$.fn.trigger不能使用。
     proxy: function(e) {}, // 真正绑定事件时的回调函数，里面判断调用del或者fn
     sel: undefined // 要进行事件代理时传入的selector
     }]
     }
     specialEvents   ：creatEvent生成模拟事件时，click、mousedown、mouseup和mousemove使用'MouseEvents'参数
     focusinSupported：是否支持focusin和focusout(此处是因为focusin、focusout和mouseover、mouseout支持冒泡而相对的那两个不支持)
